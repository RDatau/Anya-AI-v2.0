...
const ChatView: React.FC<ChatViewProps> = ({ 
      ...
      if (rawFirstResponse.toUpperCase().includes('[CAPTION:')) {
        ...
        try {
          ...
          if (isQuota) {
            ...
            finalDisplayText = `${cleanResponseText(rawFirstResponse)}\n\nAduh sorry, fotonya nyangkut di sistem... udah kayak memek gue aja yang ketat banget. Heheh.. Padahal gue tuh mau ngirim foto gue yang ${originalCaption} tapi sistemnya rewel banget nih.. 💦`;
          } else {
      ...
      >
        {isImg ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-blue-500/20">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-white" viewBox="0 0 20 20" fill="currentColor"><path d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 1.343-3 3s1.343 3 3 3 3-1.343 3-3V7.82l8-1.6V11.114A4.369 4.369 0 0015 11c-1.657 0-3 1.343-3 3s1.343 3 3 3 3-1.343 3-3V3z" /></svg>
          </div>
        ) : (
        ...
        {onRemove && (
          <button onClick={(e) => { e.stopPropagation(); onRemove(); }} className="absolute top-1 right-1 p-1 bg-black/60 rounded-lg text-white opacity-0 group-hover/att:opacity-100 transition-all">
  ...
  return (
    <div className="w-full h-full flex flex-col p-4 md:p-8 max-w-6xl mx-auto relative overflow-hidden" onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={e => { e.preventDefault(); setIsDragging(false); handleFiles(e.dataTransfer.files); }}>
      <header className="flex items-center justify-between p-3 md:p-4 rounded-full mb-6 shadow-xl shrink-0" style={glassStyles}>
        <div className="flex items-center gap-4">
          ...
          <div className="relative group/avatar cursor-pointer" onClick={() => setShowProfilePreview(true)}>
            <img src={config.profilePic || ''} className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover border-2 border-white/20" alt="Avatar" />
            <div className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-black"></div>
          </div>
          <div>
            <h2 className="font-bold text-base md:text-lg leading-tight text-white">{config.name}</h2>
            <p className="text-[9px] text-green-400 font-black uppercase tracking-widest">Online</p>
          </div>
        </div>
        <div className="flex gap-2 items-center">
          <button onClick={onEdit} className="p-3 bg-white/5 hover:bg-white/20 rounded-full transition-all border border-white/10 text-pink-400"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924-1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg></button>
          <button onClick={onCall} className="px-5 py-2.5 bg-pink-500 hover:bg-pink-600 rounded-full font-black transition-all text-xs uppercase shadow-lg shadow-pink-500/30">Call</button>
        </div>
      </header>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-6 px-2 mb-4 custom-scrollbar">
        {activeThread.map((m, idx) => (
          <div key={m.id} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2 duration-300`}>
            <div className={`relative group/bubble max-w-[85%] p-5 shadow-xl transition-all ${m.role === 'user' ? 'bg-pink-500/90 rounded-[30px] rounded-tr-none' : 'rounded-[30px] rounded-tl-none'}`} style={m.role === 'agent' ? glassStyles : {}}>
              <button onClick={() => { setEditingId(m.id); setEditText(m.text); }} className={`absolute ${m.role === 'user' ? '-left-10' : '-right-10'} top-2 p-2 bg-white/5 hover:bg-white/20 rounded-full text-white/20 hover:text-white opacity-0 group-hover/bubble:opacity-100 transition-all`}><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
              ...
              {editingId === m.id ? (
                <div className="space-y-3">
                  <textarea className="w-full bg-black/20 border border-white/20 rounded-xl p-3 outline-none text-sm text-white resize-none min-h-[100px]" value={editText} onChange={e => setEditText(e.target.value)} autoFocus />
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => setEditingId(null)} className="text-[10px] font-bold uppercase text-white/40 px-3 py-1">Batal</button>
                    <button onClick={() => handleEditSave(m)} className="text-[10px] font-bold uppercase bg-white text-pink-600 px-3 py-1 rounded-lg">Update</button>
                  </div>
                </div>
              ...
              {m.role === 'agent' && (
                <div className="flex gap-2 mt-4 flex-wrap items-center">
                  <button onClick={() => handleListen(m)} disabled={loadingAudioId === m.id} className={`text-[10px] font-bold uppercase py-2 px-4 rounded-xl transition-all border flex items-center gap-2 ${playingAudioId === m.id ? 'bg-pink-500/20 border-pink-500/30 text-pink-400' : 'bg-white/10 hover:bg-white/20 border-white/10 text-white/80'}`}>{loadingAudioId === m.id ? '...' : playingAudioId === m.id ? 'Playing' : 'Listen'}</button>
                  {idx === activeThread.length - 1 && ( <button onClick={() => handleSend(undefined, true)} disabled={isTyping} className="text-[10px] font-bold uppercase py-2 px-4 bg-pink-500/20 hover:bg-pink-500/30 rounded-xl transition-all border border-pink-500/20 text-pink-400">Regen</button> )}
                </div>
              )}
            </div>
        ...
        {isTyping && ( <div className="flex justify-start"><div className="rounded-[30px] rounded-tl-none p-5 flex flex-col gap-2 shadow-xl min-w-[150px]" style={glassStyles}><div className="flex gap-2"><div className="w-2 h-2 bg-pink-500 rounded-full animate-bounce"></div><div className="w-2 h-2 bg-pink-500 rounded-full animate-bounce [animation-delay:0.2s]"></div><div className="w-2 h-2 bg-pink-500 rounded-full animate-bounce [animation-delay:0.4s]"></div></div><span className="text-[10px] font-black text-pink-400 italic">{loadingStatus}</span></div></div> )}
      </div>

      <div className="flex flex-col gap-2 w-full max-w-6xl mx-auto shrink-0 px-2 pb-4 md:pb-6">
        {attachedFiles.length > 0 && (
          <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar p-3 bg-white/5 backdrop-blur-3xl rounded-[25px] border border-white/10 mb-2">
            {attachedFiles.map((file, i) => (
              <AttachmentPreview key={i} attachment={file} onRemove={() => removeAttachment(i)} />
            ))}
          </div>
        )}
        <footer className="relative flex items-center gap-3 p-1.5 md:p-2 rounded-full shadow-2xl transition-all group" style={glassStyles}>
          ...
          <input type="text" placeholder={`Bisikin ke ${config.name}...`} className="flex-1 bg-transparent outline-none py-3 text-sm font-semibold text-white placeholder:text-white/20" value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} />
          <button onClick={() => handleSend()} disabled={(!inputText.trim() && attachedFiles.length === 0) || isTyping} className="p-3 bg-gradient-to-br from-pink-500 to-purple-600 rounded-full transition-all active:scale-95 disabled:opacity-20 shadow-lg border border-white/10 flex items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white" viewBox="0 0 20 20" fill="currentColor"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" /></svg></button>
        </footer>
      </div>

      {isDragging && (
        <div className="absolute inset-0 z-[150] bg-black/60 backdrop-blur-2xl flex flex-col items-center justify-center pointer-events-none p-6 animate-in fade-in duration-300">
          <div className="w-full h-full rounded-[40px] border-4 border-dashed border-pink-500/50 bg-pink-500/5 flex flex-col items-center justify-center gap-4">
             <div className="p-8 bg-pink-500 rounded-full animate-bounce">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2-2v12a2 2 0 002 2z" /></svg>
             </div>
             <p className="text-xl font-black text-pink-400 italic">Masukin filenya di sini sayang... mmmh... 💦</p>
          </div>
        </div>
      )}

      {showProfilePreview && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-3xl" onClick={() => setShowProfilePreview(false)}>
          <div className="relative max-w-lg w-full aspect-square" onClick={e => e.stopPropagation()}>
            <img src={config.profilePic || ''} className="w-full h-full object-cover rounded-[40px] shadow-2xl border-2 border-white/10" alt="Profile" />
            <div className="absolute top-4 right-4 flex flex-col gap-3">
              <button onClick={() => setShowProfilePreview(false)} className="bg-white/10 backdrop-blur-md text-white p-3 rounded-2xl border border-white/20"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg></button>
              <button onClick={() => profileInputRef.current?.click()} className="bg-purple-600/80 backdrop-blur-md text-white p-3 rounded-2xl border border-white/20"><input type="file" ref={profileInputRef} className="hidden" accept="image/*" onChange={handleProfilePicChange} /><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg></button>
              <button onClick={() => setConfig({ ...config, profilePic: defaultProfilePic })} className="bg-blue-600/80 backdrop-blur-md text-white p-3 rounded-2xl border border-white/20"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg></button>
            </div>
          </div>
        </div>
      )}
      ...
      {previewMedia && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/95 backdrop-blur-3xl" onClick={() => setPreviewMedia(null)}>
          <div className="relative max-w-4xl w-full flex flex-col items-center gap-4 animate-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
            <div className="w-full flex justify-end">
               <button onClick={() => setPreviewMedia(null)} className="p-3 bg-white/10 hover:bg-red-500 rounded-2xl text-white transition-all"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <div className="w-full rounded-[30px] overflow-hidden border border-white/10 shadow-2xl bg-black/40 flex items-center justify-center">
               {previewMedia.mimeType.startsWith('image/') ? ( <img src={previewMedia.data} className="w-full h-auto max-h-[75vh] object-contain" /> ) : previewMedia.mimeType.startsWith('video/') ? ( <video src={previewMedia.data} controls autoPlay className="w-full h-auto max-h-[75vh]" /> ) : ( <div className="p-20 flex flex-col items-center gap-4"><svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-20 text-white/10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><p className="text-white/40 font-black uppercase text-xs tracking-widest">Preview Tidak Tersedia</p></div> )}
            </div>
          </div>
        </div>
      )}
